...
Questo è il corredo pratico del libro: notebook per Google Colab, dataset di esempio, esercizi, soluzioni, test, roadmap e materiali di studio. Tutto è pensato per chi parte da zero e vuole imparare facendo, direttamente nel browser.

---

## Cosa trovi in questo kit

- **15 notebook Colab** (`.ipynb`): 2 generali (esercizi e soluzioni) + 13 mini-progetti
- **12 dataset** di esempio (`.csv`) collegati ai mini-progetti
- **150 esercizi** con soluzioni commentate
- **Test finale** di 200 domande con soluzioni
- **202 prompt** pronti per usare l'AI come tutor di studio
- **Glossario di 64 errori** comuni
- **Roadmap** dei percorsi dopo le basi
- **Registro progressi** in Excel
- **Attestato** di completamento

---

## Come aprire i notebook in Google Colab

I notebook sono pensati per essere eseguiti in Google Colab, dal browser, usando la versione gratuita di Google Colab per gli esercizi di base.

### Modo rapido: pulsante "Apri in Colab"

Nella tabella qui sotto, clicca su **Apri in Colab** accanto al notebook che ti interessa. Si aprirà direttamente in Colab.

Per modificarlo e tenerne una copia tua: in Colab vai su **File > Salva una copia in Drive**. Lavorerai sulla tua copia personale, l'originale resta intatto qui sul repository.

### Modo manuale: da GitHub

1. Apri [Google Colab](https://colab.research.google.com) e accedi con il tuo account Google.
2. Menu **File > Apri notebook**.
3. Scheda **GitHub**.
4. Incolla: `responsabile2014-alt/python-da-zero-notebook`
5. Scegli il notebook dall'elenco (sono nella cartella `bonus-ai-ready-premium-kit/06_Notebook_Colab`).
6. Per tenerne una copia: **File > Salva una copia in Drive**.

---

### Nota sugli esercizi interattivi

Alcuni notebook, in particolare il notebook delle soluzioni, contengono esercizi interattivi con `input()`. In questi casi Google Colab aspetta che tu inserisca un valore da tastiera.

Per questo motivo, soprattutto all'inizio, è consigliato eseguire una cella alla volta invece di usare subito "Esegui tutto". In questo modo puoi leggere la consegna, inserire l'input richiesto, osservare l'output e capire meglio il funzionamento del codice.

## I 15 notebook - Apri in Colab
...
| Tipo | A cosa serve |
|------|--------------|
| **Notebook esercizi** | `00_Workbook_150_Esercizi.ipynb`: i 150 esercizi con spazi da completare. La tua palestra. |
| **Notebook soluzioni** | `00_Soluzioni_150_Commentate.ipynb`: le 150 soluzioni commentate. Da usare DOPO aver provato. |
| **Notebook mini-progetto** | Da `01_` a `13_`: piccoli progetti completi, pensati per essere eseguiti in Colab. |
| **Dataset CSV** | Cartella `07_Dataset`: i dati di esempio su cui lavorano esercizi e progetti. |

Metodo consigliato: prima provi un esercizio nel notebook esercizi, poi controlli nel notebook soluzioni. I mini-progetti li esegui per vedere Python applicato a casi reali.

---

## Scaricare tutti i materiali (Download ZIP)

Per avere tutto sul tuo computer:

1. Vai sulla pagina del repository: https://github.com/responsabile2014-alt/python-da-zero-notebook
...
```
bonus-ai-ready-premium-kit/
  00_Guida_Uso_Kit/        Come usare il kit
  01_Guida_AI_Ready/       Dove porta Python (dati, automazione, AI)
  02_AI_Senza_Illusioni/   Usare l'AI con metodo
  03_Prompt_Pack/          202 prompt pronti per studiare
  04_Workbook/             150 esercizi
  05_Soluzioni/            150 soluzioni + glossario errori
  06_Notebook_Colab/       15 notebook .ipynb
  07_Dataset/              12 dataset .csv
  08_Test_Finale/          200 domande con soluzioni
  09_Roadmap/              I percorsi dopo le basi
  10_Registro_Progressi/   Foglio Excel per monitorare
  11_Attestato/            Attestato di completamento
  12_GitHub_Amazon/        README e testi di pubblicazione
  13_QA_Verifica/          Verifica finale dei materiali
  README.md
```
...
- Un account Google e un browser. Niente da installare.
- I notebook usano solo Python di base e il modulo standard `csv`: nessuna libreria esterna.
- I mini-progetti ricreano da soli i propri dati: funzionano in Colab senza caricamenti manuali.
- L'interfaccia di Google Colab può cambiare nel tempo: i nomi dei menu potrebbero variare leggermente.

---

*Questi materiali sono il corredo pratico del libro "Python da Zero con Google Colab". Sono pensati per lo studio personale.*

Buono studio. Un passo alla volta, un notebook alla volta: è così che si impara davvero.
